...
export async function downloadTrainingServiceReport(queryString: string) {
  const suffix = queryString ? `?${queryString}` : '';
  const url = `${getApiUrl()}/reports/training/export${suffix}`;
  const filename = `Training_Service_Report_${new Date().toISOString().split('T')[0]}.xlsx`;
